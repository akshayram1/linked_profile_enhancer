...
"""
    ...
    for category in suggestions.keys():
        if category == 'ai_generated_content':
            toc_items.append("- [🤖 AI-Generated Content Suggestions](#ai-generated-content-suggestions)")
        else:
            category_name = category.replace('_', ' ').title()
            toc_items.append(f"- [📋 {category_name}](#{category.replace('_', '-').lower()})")
    ...
    for category, items in suggestions.items():
        if category == 'ai_generated_content':
            markdown_content += "## 🤖 AI-Generated Content Suggestions\n\n"
...
def display_suggestions(suggestions):
    ...
    with col2:
        st.markdown("*💡 Click the download button to save all suggestions as a markdown file for easy reference and implementation.*")
